...
export default function Dashboard({ view }: { view: string }) {
  ...
  const openReviews = (title: string, selected = reviews) => setDrawer({ title, rows: selected.map((r) => ({ id: r.id, meta: `${r.testerId} · ${COURSE_LABELS[r.course]} · ${r.device}`, text: `${r.completion}; submitted ${new Date(r.submittedAt).toLocaleDateString()}`, source: r.sourceId })) });
